...
@pytest.mark.parametrize(
    ("command", "fmt", "result"),
    [
        (DailyReportCommand, "pdf", True),
        (DailyReportCommand, "csv", True),
        (WeeklyReportCommand, "pdf", False),  # NOTE: Problems with Toggl API endpoint.
        (WeeklyReportCommand, "csv", True),
        (MonthlyReportCommand, "pdf", True),
        (MonthlyReportCommand, "csv", True),
    ],
)
def test_report_dump(command, fmt, dummy_ext, tmp_path, result):
    cmd = command(dummy_ext)
    tmp_path = Path(tmp_path)
    now = datetime.now(tz=timezone.utc)
    handle = cmd.handle([], day=now, format=fmt, path=tmp_path)
